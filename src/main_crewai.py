...
def _build_risk_review_workpaper(identified_risks: pd.DataFrame) -> pd.DataFrame:
    ...
    risk_type = workpaper["risk_type"].astype(str).str.strip()
    financial_risk_mask = risk_type.str.casefold().isin(
        {risk_type.casefold() for risk_type in FINANCIAL_GUARDRAIL_RISK_TYPES}
    )
    financial_risk_counts = financial_risk_mask.groupby(workpaper["entity_name"]).transform(
        "sum"
    )
    requires_human_review = (
        ...
        | risk_type.str.casefold().eq("manual risk flag")
        | (financial_risk_mask & financial_risk_counts.gt(1))
    )
...
def main() -> None:
    ...
    results = audit_crew.run(financial_file=args.financial_file)
    risk_discovery_paths = _export_risk_discovery_outputs(
        results["identified_risks"],
        Path(config.output_directory),
    )
    financial_status = results["financial_status"]
